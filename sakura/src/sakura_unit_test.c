...
START_TEST ( test_sakura_array_new )
{
   ...
   for ( i=-2 ; i<12 ; i++ )
      for ( n=0 ; n<4 ; n++ )
         fail_unless( eh_compare_dbl(a->c_grain[i][n],0,1e-12) , "Member c_grain not initialized" );
...
START_TEST ( test_sakura_next_c_and_h )
{
...

   {
      ...
      for ( i=0 ; i<5 ; i++ )
         u_settling[i] = sakura_settling_velocity( rho_grain[i] , equiv_dia[i] , S_RHO_WATER , S_MU_WATER );
