...
% information is in ASCII and the data is written as chars.
   ...
   hdr=read_property_header_old(fid);
   
   if ( ~isempty(col_no) )
      data = nans( [hdr.n_rows length(col_no)]);
      data = get_property_column( fid , col_no );
   else
      data = nans([hdr.n_rows hdr.n_cols]);
      if ( ascii==0 )
         data = fread(fid,[hdr.n_rows hdr.n_cols],'uchar');
      else
         format = [ repmat( '%d ' , 1 , hdr.n_rows ) '\n' ];
         data = fscanf( fid , format , [hdr.n_rows hdr.n_cols] );
      end
   end
...
function col = get_property_column( fid , col_no )

rewind(fid);
hdr = read_property_header(fid);
for i=1:length(col_no)
   seek_property_column( fid , col_no(i) );
   col(:,i) = fread( fid , hdr.n_rows , 'char' );
end

function seek_property_column( fid , col_no )
