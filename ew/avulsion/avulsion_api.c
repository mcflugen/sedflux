...
BMI_AVULSION_Update(void* self)
{
    ...
    if (BMI_AVULSION_Get_current_time(self, &now) == BMI_SUCCESS &&
        BMI_AVULSION_Update_until(self,
            now + ((AvulsionModel*)self)->time_step) == BMI_SUCCESS) {
        return BMI_SUCCESS;
    } else {
        return BMI_FAILURE;
    }
...

    {
        ...
        if (g_ascii_strcasecmp(val_string,
            ...
            g_ascii_strcasecmp(val_string,
                "channel_outflow_end_suspended_sediment__discharge") == 0) {
            src = self->discharge;
        } else if (g_ascii_strcasecmp(val_string,
                "channel_outflow_end__location_model_x_component") == 0) {
            src = self->mouth_x;
        } else if (g_ascii_strcasecmp(val_string,
                "channel_outflow_end__location_model_y_component") == 0) {
            src = self->mouth_y;
        } else if (g_ascii_strcasecmp(val_string,
                "channel_inflow_end_to_channel_outflow_end__angle") == 0) {
