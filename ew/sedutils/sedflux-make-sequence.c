...
main(int argc, char* argv[])
{
    GOptionContext* context =
        g_option_context_new("Create a sequence grid file for sedflux-3d");
    GError*         error   = NULL;
    FILE*           fp_out  = stdout;
    ...
    eh_message("Byte order  : %s",
        byte_order == G_BIG_ENDIAN ? "Big-endian" : "Little-endian");
    eh_message("Format      : %s",
        "<32-bit-integer=NY> <32-bit-integer=NX> <<64-bit-double> <<64-bit-double*NX*NY>... EOF>>");
