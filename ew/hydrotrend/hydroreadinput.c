...
hydroreadinput()
{
    ...
    for (ep = 0; ep < nepochs; ep++) {
         ...
         *--------------------------------------------------------------------------*/
        for (jj = 0; jj < 12; jj++) {
            fscanf(fidinput, "%3s %lf %lf %lf %lf ", moname[jj], &Tnominal[jj][ep],
                &Tnomstd[jj][ep], &Pnominal[jj][ep], &Pnomstd[jj][ep]);
            fgets(chs, 120, fidinput);
            Pnominal[jj][ep] /= 1000;
            Pnomstd[jj][ep]  /= 1000;
        }
        ...
        if (webflag == 1) {
            ...
            if ((totyears > 10000) && (timestep[0] != 'y')) {
                ...
                fprintf(stderr,
                    "      So the output interval option of the inputfile is overruled. \n ");
            }
