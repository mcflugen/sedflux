...
sakura_array_print_data(Sakura_array* a, Sakura_const_st* c)
{
    ...
    if (c->data_fp && c->data_id) {
        ...
        for (id = c->data_id ; *id >= 0 ; id++) {
            switch (*id) {
                case SAKURA_VAR_VELOCITY:
                    data = a->u;
                    break;

                case SAKURA_VAR_DEPTH:
                    data = a->h;
                    break;

                case SAKURA_VAR_WIDTH:
                    data = a->c;
                    break;
...
sakura_node_set(Sakura_node* x, double u, double c, double h, double* c_grain, gint len)
{
    ...
    if (x) {
        ...
        if (c_grain) {
            x->c_grain = eh_dbl_array_copy(x->c_grain, c_grain, len);
        } else if (x->c_grain) {
            x->c_grain = eh_dbl_array_set(x->c_grain, len, 0.);
        } else {
            x->c_grain = eh_new0(double, len);
        }
...
double
sakura_erode_depth(double rho_f, double u, double dt, double sua, double sub,
    double c_drag)
{
    double e = 0;
...
Sakura_array*
sakura_next_c_grain(Sakura_array* a_next, Sakura_array* a_last, double* u, gint i,
    double dt, Sakura_sediment* sed)
{
    eh_require(a_next);
    eh_require(a_last);
    eh_require(sed);
...
double
sakura_erode(Sakura_array* a, Sakura_sediment* sed, double* u, gint i, double dt,
    Sakura_const_st* c)
{
    double ero = 0;
    ...
    if (a && sed && c && dt > 0) {
        ...
        e_tot      = sakura_erode_depth(rho_f, .5 * (u[i] + u[i + 1]), dt, c->sua, c->sub,
                c->c_drag);
        ...
        for (n = 0 ; n < n_grains ; n++) {
            ...
            if (e_grain > 0) {
                e_grain = c->remove(c->remove_data, a->x[i], &sediment);
            }
...
double
sakura_deposit(Sakura_array* a, Sakura_sediment* sed, gint i, double dt,
    Sakura_const_st* c)
{
    double dep = 0;
    ...
    if (a && sed && dt > 0) {
        if (a->x[i] > c->dep_start) {
            ...
            for (n = 0 ; n < n_grains ; n++) {
                ...
                // Meters of sediment
                if (a->h[i] <= small_h) {
                    d_grain = a->h[i] / dt           * a->c_grain[i][n];
                } else {
                    d_grain = sed->u_settling[n] * Ro * a->c_grain[i][n];
                }
                ...
                if (d_grain > 0) {
                    d_grain = c->add(c->add_data, a->x[i], &sediment);
                }
...
sakura_deposit_all(Sakura_array* a, Sakura_sediment* sed, Sakura_const_st* c)
{
    ...
    if (a && sed) {
        ...
        for (n = 0 ; n < n_grains ; n++) {
            f_sed[n] = 1. - (sed->rho_grain[n] - sed->rho_dep[n]) / (sed->rho_grain[n] -
                    c->rho_sea_water);
        }
        ...
        for (i = 0 ; i < a->len ; i++) {
            ...
            for (n = 0 ; n < n_grains ; n++) {
                ...
                if (vol_grain > 0) {
                    vol_grain = c->add(c->add_data, a->x[i], &sediment);
                }
...
gboolean
compute_c_grain_new(Sakura_array* a, Sakura_array* a_last, double* u, gint i, double dt,
    Sakura_const_st* c, Sakura_sediment* sed)
{
    gboolean success = TRUE;
    ...
    Sakura_const_st* Const, Sakura_sediment* sed)
{
    ...
    if (a && a_last) {
        ...
        for (n = 0, sed_rate = 0, c_new = 0; n < n_grain ; n++) {
            ...
            erode_depth     = (Const->c_drag * (1 + c_grain_new * R) * Const->rho_sea_water * um *
                    um - Const->sub)
            ...
            if (a->h[i] <= small_h) {
                flux_at_bed = a->h[i] / dt              * c_grain_new - eh_max(0, erosion[n]);
            } else {
                flux_at_bed = sed->u_settling[n] * Ro * c_grain_new - eh_max(0, erosion[n]);
            }

            if (a->x[i] < Const->dep_start) {
                flux_at_bed = 0.0;
            }
            ...
            if (dh < 0) {
                a->e[i][n] += remove_func(Const->remove_data, a->x[i], &sediment);
            } else {
                a->d[i][n] += add_func(Const->add_data, a->x[i], &sediment);
            }
            ...
            if (a->h[i] < HMIN) {
                ...
                a->c_grain[i][n] = c_grain_new - flux_at_bed * dt / a->h[i];
            }
...
gboolean
compute_next_h(Sakura_array* a_new, Sakura_array* a_last, double* u_temp, gint ind_head,
    Sakura_const_st* c)
{
    gboolean success = TRUE;
...
gboolean
compute_next_c(Sakura_array* a_new, Sakura_array* a_last, double* u_temp, gint ind_head,
    Sakura_const_st* c, Sakura_sediment* sed)
{
    gboolean success = TRUE;
    ...
    if (a && u) {
        ...
        if (ind_head < a->len) {
            ...
            if (ind_head <= 0) {
                ...
                u_head = eh_min(u_head, 1.5 * pow(G * R * a->c[ind_head - 1] * a->h[ind_head - 1],
                            1. / 3.));
            } else {
                u_head = eh_max(u[ind_head], u[ind_head - 1]);
            }
    ...
    if (dt > 0) {
...

        {
            ...
            for (t = 0., n = 0 ; t <= total_t&& success ; t += dt, n++) {
                ...
                if (success) {
                    success = calculate_mid_vel(a_mid, a_last, ind_head, c);
                }

                // Calculate c, and h at t+dt.  This is a_next.  a_next->u is not valid.
                if (success) {
                    success = compute_next_h(a_next, a_last, a_mid->u, ind_head, c);
                }

                if (success) {
                    success = compute_next_c(a_next, a_last, a_mid->u, ind_head, c, sed);
                }
                ...
                if (success) {
                    success = calculate_mid_c_and_h(a_mid, a_last, a_next);
                }

                // Calculate u at t+dt.  This is a_next->u.
                if (success) {
                    success = calculate_next_vel(a_last, a_mid, a_next, ind_head, c);
                }
                ...
                if (c->data_int > 0 && n % c->data_int == 0) {
                    sakura_array_print_data(a_last, c);
                }
        ...
        if (TRUE || g_getenv("SAKURA_DEBUG")) {
            ...
            if (mass_bal > 0) {
                eh_message("Relative error (-)       : %g (lost)", mass_bal / mass_in);
            } else {
                eh_message("Relative error (-)       : %g (gained)", mass_bal / mass_in);
            }
