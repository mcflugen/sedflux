...
gboolean
plume2d(Plume_inputs* plume_const, Plume_river river_passed, int ngrains_passed,
    Plume_sediment* sedload_passed, double dx, double** deposit, int depositLen,
    Plume_data*);
gboolean
...
Plume_data*
plume_data_init(Plume_data* data)
{
    data->x_len = 0;
    data->y_len = 0;
    data->ccnc  = NULL;
...
void
destroy_plume_data(Plume_data* grid)
{
    if (grid) {
        eh_free(grid->xval);
        eh_free(grid->yval);
...
gboolean
plume2d(Plume_inputs* plume_const, Plume_river river,
    int n_grains, Plume_sediment* sedload,
