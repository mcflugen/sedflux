...
START_TEST(test_approx_nd)
{
    ...
    fail_unless(eh_compare_dbl(inv, 0, 1e-12),
        "Inventory should be zero at large distance");
...
START_TEST(test_approx_from_file)
{
...

    {
        ...
        Sed_hydro_file f = sed_hydro_file_new(SED_HYDRO_TEST_FILE, SED_HYDRO_HYDROTREND, FALSE,
                TRUE, NULL);
        Sed_hydro river;
        double dt;
        double l;
...
main(void)
{
    ...
    if (!this_sed) {
        eh_error("%s: Unable to scan sediment file: %s", SED_SEDIMENT_TEST_FILE,
            error->message);
    }
