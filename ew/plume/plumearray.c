...
//---
int
plumearray(Plume_grid* grid, Plume_enviro* env, Plume_options* opt)
{
    ...
    //---
    zend = 1.2 * pow((-(river->u0 / mnlm) * sqrt(river->b0 / (sqpi * C1)) * log(pcrit)),
            (2.0 / 3.0));
    ...
    if (fabs(grid->ymin) > grid->ymax) {
        grid->ymax =  fabs(grid->ymin);
    }

    if (fabs(grid->ymin) < grid->ymax) {
        grid->ymin = -fabs(grid->ymax);
    }
