...
double* plume_width_averaged_inv( double* inv , double* x , gssize len ,
...
double* plume_width_averaged_deposit_nd( double* dep , double* x , gssize len , double l )
{
...

   {
      ...
      for ( i=1 ; i<len ; i++ )
      {
         ...
         dep[i-1] = eh_integrate_with_data( plume_width_averaged_inventory_at_helper , x[i-1] , x[i] , data )
                  / ( x[i] - x[i-1] ) * 2;
      }
   }
...
double* plume_inv( double* inv , double* x   , double* y , gssize len ,
                   double  l   , double  i_0 , Sed_hydro r )
{
   eh_return_val_if_fail( x     , NULL );
...
double plume_v_bar_established( double x , double y )
{
   ...
   return sqrt(PLUME_XA/x)/PLUME_SIGMA*( s*exp( -pow(s,2.) ) - .25*EH_SQRT_PI*erf( s ) );
}
...
//   return plume_v_bar_established(x,y);
   return exp( -pow(w,2) ) * ( w/PLUME_SIGMA - .5/PLUME_XA ) + .5/PLUME_XA*(1-G_SQRT2*erf(w));
}
...
//            t[n] = dep[i] * i_0 * pow(sed_hydro_width(r),2) / (dy*sed_hydro_width(r)*(x[i+1]-x[i])*rho)*sed_type_lambda( sed_sediment_type(s,n) );
            t[n] = dep[i] * i_0 / (rho*dy) * sed_type_lambda( sed_sediment_type(s,n) ) * sed_hydro_width(r)*dt;
//            t[n] = dep[i] * i_0 / rho * dt / dy;
