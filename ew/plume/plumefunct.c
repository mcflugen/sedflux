...
/* allocate a float matrix with subscript range m[nrl..nrh][ncl..nch] */
{
    ...
    if (!m) {
        eh_error("allocation failure 1 in matrix()");
    }
    ...
    if (!m[nrl]) {
        eh_error("allocation failure 2 in matrix()");
    }
...
double**
new_dmatrix(long n_rows, long n_cols)
{
    int i;
    double** m;
...
/* allocate a float 3tensor with range t[nrl..nrh][ncl..nch][ndl..ndh] */
{
    ...
    if (!t) {
        eh_error("allocation failure 1 in f3tensor()");
    }
    ...
    if (!t[nrl]) {
        eh_error("allocation failure 2 in f3tensor()");
    }
    ...
    if (!t[nrl][ncl]) {
        eh_error("allocation failure 3 in f3tensor()");
    }
...
double***
new_d3tensor(long n_rows, long n_cols, long n_dep)
{
    int i, j;
    double*** t;
