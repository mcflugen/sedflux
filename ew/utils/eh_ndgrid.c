...
double*
eh_ndgrid_x(Eh_ndgrid g, gssize dim)
{
...
gssize
eh_ndgrid_n(Eh_ndgrid g, gssize dim)
{
    eh_require(dim < g->n_dim);
    return g->size[dim];
}
...
Eh_ndgrid
eh_ndgrid_malloc(gssize n_dim, gssize el_size, ...)
{
    gssize n;
    gssize n_el = 1;
    Eh_ndgrid g;
...
Eh_ndgrid
eh_reshape_ndgrid(Eh_ndgrid g, gssize* new_size, gssize new_n_dim)
{
    gssize k_old, k_new;
    gssize n;
...
gssize
eh_ndgrid_sub_to_id(gssize* size, gssize* sub, gssize n_dim)
{
    gssize n;
    gssize* k;
    gssize id;
...
gssize*
eh_ndgrid_id_to_sub(gssize* size, gssize id, gssize n_dim)
{
    gssize n;
    gssize* k;
    gssize* sub;
...
gssize
eh_ndgrid_write(FILE* fp, Eh_ndgrid g)
{
    gssize n, n_x;
