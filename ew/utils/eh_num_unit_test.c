...
double
linear_f(double x, gpointer data)
{
    return 2 * x + 1;
}

double
exp_f(double x, gpointer data)
{
    return exp(x);
}
...
double
inventory(double x, double s, double l)
{
    double i;
...
double
inventory_f(double x, gpointer data)
{
    return inventory(((User_data*)data)->y, ((User_data*)data)->c * x,
            ((User_data*)data)->l);
}
...
START_TEST(test_trapazoid)
{
    ...
    fail_unless(fabs(sum - (exp(1) - exp(0))) < 1e-4,
        "Integration of EXP failed for trapzd");

    sum = qtrap(exp_f, 0, 1, NULL);
    fail_unless(fabs(sum - (exp(1) - exp(0))) < 1e-4,
        "Integration of EXP failed for qtrap");
...
START_TEST(test_integrate)
{
    ...
    fail_unless(fabs(sum - (exp(1) - exp(0))) < 1e-4,
        "Integration of EXP failed for integrate");
...
START_TEST(test_diff)
{
    ...
    for (i = 0 ; i < len - 1 ; i++) {
        fail_unless(eh_compare_dbl(fx[i], f[i + 1] - f[i], 1e-12),
            "First derivative calculated incorrectly");
    }

    for (i = 1 ; i < len - 1 ; i++) {
        fail_unless(eh_compare_dbl(fxx[i], f[i + 1] - 2 * f[i] + f[i - 1], 1e-12),
            "Second derivative calculated incorrectly");
    }

    for (i = 1 ; i < len - 2 ; i++) {
        fail_unless(eh_compare_dbl(fxxx[i], f[i + 2] + 3 * f[i] - 3 * f[i + 1] - f[i - 1],
                1e-12), "Third derivative calculated incorrectly");
    }

    for (i = 2 ; i < len - 2 ; i++) {
        fail_unless(eh_compare_dbl(fxxxx[i],
                f[i + 2] + 6 * f[i] + f[i - 2] - 4 * f[i + 1] - 4 * f[i - 1], 1e-12),
            "Fourth derivative calculated incorrectly");
    }

    fail_unless(eh_compare_dbl(fx[len - 1], fx[len - 1], 1e-12),
        "First difference not calculated at n=N-1");

    fail_unless(eh_compare_dbl(fxx[0], fxx[1], 1e-12),
        "Second difference not calculated at n=0");
    fail_unless(eh_compare_dbl(fxx[len - 1], fxx[len - 2], 1e-12),
        "Second difference not calculated at n=0");
...
START_TEST(test_binomial_coef)
{
    ...
    fail_unless(eh_compare_dbl(c, 2598960, 1e-12),
        "Binomial coefficient calculated incorrectly");
