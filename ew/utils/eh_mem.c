...
eh_mem_profile_fprint(FILE* fp)
{
    ...
    fprintf(fp, "Bytes freed        : %12ld (%f%%)\n", total_free,
        total_free * 100. / total);
    fprintf(fp, "Bytes in use       : %12ld (%f%%)\n", total - total_free,
        (total - total_free) * 100. / total);
}
...
add_to_linked_list(Heap_Prefix);
G_GNUC_INTERNAL void
remove_from_linked_list(Heap_Prefix);
G_GNUC_INTERNAL gboolean
verify_heap_pointer(void*);
G_GNUC_INTERNAL gsize
render_desc(Heap_Prefix, char*);

#include <stdlib.h>

gpointer API_ENTRY
eh_malloc(gsize w_size,
    Class_Desc desc,
    const char* file,
    int line_no)
{
...
gpointer API_ENTRY
eh_malloc_c_style(gsize w_size)
{
    if (w_size) {
        gpointer mem;
        ...
        eh_error("Could not allocate %d bytes.", (gint)w_size);
    }
...
gpointer API_ENTRY
eh_calloc_c_style(gsize n_blocks, gsize n_block_bytes)
{
    if (n_blocks * n_block_bytes) {
        ...
        eh_error("Could not allocate %d bytes.", (gint)(n_blocks * n_block_bytes));
    }
...
gpointer API_ENTRY
eh_realloc(gpointer old, gsize w_size,
...
gpointer API_ENTRY
eh_realloc_c_style(gpointer old, gsize w_size)
{
    if (w_size) {
        gpointer mem;
...
void
add_to_linked_list(Heap_Prefix add)
{
    if (heap_head) {
...
void
remove_from_linked_list(Heap_Prefix remove)
{
        ...
        if (eh_is_ptr_ok(mem)) {
            ...
            if (prefix->mem == mem) {
                if (prefix->postfix->prefix == prefix) {
                    ...
                    fprintf(stderr, "(%p: %p) Pointer overwrite\n", mem, mem);
                }
            } else {
                fprintf(stderr, "(%p: %p) Pointer underwrite\n", mem, mem);
            }
        } else {
            fprintf(stderr, "(%p) Pointer is not aligned\n", mem);
        }
    }
...
void API_ENTRY
report_win_assert(char* file_name, int line_no)
{
