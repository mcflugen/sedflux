...
Eh_data_record
eh_data_record_destroy(Eh_data_record p)
{
    if (p) {
...
void
eh_data_record_print(Eh_data_record p,
    char* rec_name,
    char* delim,
    gboolean row_major,
...
int
eh_data_record_size(Eh_data_record p, int dim)
{
...
Eh_symbol_table
eh_data_record_table(Eh_data_record p)
{
    return p->t;
}
...
void
eh_data_record_set_row(Eh_data_record p, int row, double* a)
{
...
void
eh_data_record_add_row(Eh_data_record p, double* a)
{
    eh_grid_add_row(p->g, a);
}

void
eh_data_record_add_column(Eh_data_record p, double* a)
{
    eh_grid_add_column(p->g, a);
}

void
eh_data_record_add_label(Eh_data_record p, char* label, char* value)
{
    eh_symbol_table_insert(p->t, label, value);
}

void
eh_data_record_interpolate_rows(Eh_data_record p, gssize row, double* y, gssize new_len)
{
    eh_require(p);
    eh_require(row >= 0);
...
Eh_data_record*
eh_data_record_scan_file(const char* file, const char* delim, int fast_dim,
    gboolean with_header, GError** error)
{
...
Eh_data_record
eh_data_record_scan(GScanner* s, const char* delim, int fast_dim, gboolean with_header)
{
    Eh_data_record ans;
