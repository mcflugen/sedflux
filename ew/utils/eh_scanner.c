...
void
__eh_symbol_table_insert(char* key, char* value, Symbol_table* t)
{
    eh_symbol_table_insert(t, g_strdup(key), g_strdup(value));
}

Symbol_table*
eh_symbol_table_dup(Symbol_table* t)
{
    return eh_symbol_table_copy(NULL, t);
}

Symbol_table*
eh_symbol_table_copy(Symbol_table* dest, Symbol_table* src)
{
    if (!dest) {
...
Symbol_table*
eh_merge_symbol_table(Symbol_table* table1, ...)
{
    va_list args;
...
void
eh_symbol_table_insert(Symbol_table* t, char* key, char* value)
{
    g_hash_table_insert((GHashTable*)t, g_strdup(key), g_strdup(value));
}

void
eh_symbol_table_replace(Symbol_table* t, char* key, char* value)
{
...
void
eh_print_symbol_table(Symbol_table* t, FILE* fp)
{
    g_hash_table_foreach(t, (GHFunc)&eh_print_symbol, fp);
}

void
eh_print_symbol_table_aligned(Symbol_table* t, FILE* fp)
{
    int max_len = 0;
    aligned_st st;
...
gssize
eh_symbol_table_size(Symbol_table* t)
{
    return g_hash_table_size(t);
}

Symbol_table*
eh_destroy_symbol_table(Symbol_table* t)
{
    if (t) {
...
void
eh_free_symbol_table_labels(Symbol_table* t)
{
    if (t) {
...
GHashTable*
eh_scan_record_file(const char* filename)
{
    GScanner* s;
    char* record_name;
...
void
eh_destroy_record_file(GHashTable* table)
{
    g_hash_table_destroy(table);
}

Symbol_table*
eh_scan_record_file_for(const char* filename, const char* rec_name)
{
    GHashTable* records;
    GSList* list;
    Symbol_table* a;
...
Symbol_table*
eh_scan_key_file_for(const gchar* file,
    const gchar* name,
    Symbol_table* tab)
{
...
GPtrArray*
eh_scan_data_file(const char* filename,
...
GPtrArray*
eh_scan_data_record(GScanner* s, const char* delimeter, gboolean row_major,
    GPtrArray* data)
{
    double d_val;
    int i, n_rows;
...
GPtrArray*
eh_print_data_table(GPtrArray* data, const char* delimeter, gboolean row_major,
    FILE* fp)
{
    int i, j, n;
    GArray* row;
...
#endif
gboolean
eh_scanner_eol(GScanner* s);

double*
eh_scan_ascii_data_line_dbl(GScanner* s, const char* delim,  gssize* len)
{
    double* row = NULL;
    gssize n_cols = 0;
...
char*
eh_scan_ascii_data_line(GScanner* s)
{
    ...
    while (g_scanner_get_next_token(s) != '\n'
        ...
        if (g_scanner_cur_token(s) == G_TOKEN_STRING) {
            ...
            g_scanner_unexp_token(s, G_TOKEN_STRING, NULL, NULL, NULL, "eh_scan_ascii_data_line",
                TRUE);
...
char*
eh_scan_next_record(GScanner* s, Eh_symbol_table symbol_table)
{
    char* record_name;
...
eh_scan_label(GScanner* s)
{
    ...
    char* nth   = g_strconcat(G_CSET_a_2_z, G_CSET_A_2_Z, " _0123456789", G_CSET_LATINS,
            G_CSET_LATINC, NULL);
...
char*
eh_seek_record_start(GScanner* s)
{
    gchar* ans;
    char c[2];
...
void
eh_free_data_record(Eh_data_record* p)
{
...
void
eh_print_data_record(Eh_data_record* p, char* rec_name, char* delimeter,
    gboolean row_major, gboolean with_header, FILE* fp)
{
    if (with_header) {
        fprintf(fp, "[ %s ]\n", rec_name);
...
Symbol_table*
eh_get_data_record_sym_table(Eh_data_record* p)
{
    return p->symbol_table;
}

GArray*
eh_get_data_record_row(Eh_data_record* p, int row)
{
...
void
eh_set_data_record_row(Eh_data_record* p, int row, GArray* a)
{
    g_ptr_array_index(p->data, row) = a;
}

void
eh_add_data_record_row(Eh_data_record* p, GArray* a)
{
    g_ptr_array_add(p->data, a);
}

void
eh_add_data_record_label(Eh_data_record* p, char* label, char* value)
{
    eh_symbol_table_insert(p->symbol_table, label, value);
}

void
eh_interpolate_data_record_rows(Eh_data_record* p, int row, GArray* x)
{
    GArray* x1, *y1, *y;
...
Eh_record_file*
eh_open_record_file(const char* filename)
{
...
void
eh_close_record_file(Eh_record_file* rec_file)
{
    if (rec_file) {
...
Eh_symbol_table*
eh_get_record_from_record_file(Eh_record_file* rec_file, const char* name)
{
...
char*
eh_get_value_from_record_file(Eh_record_file* rec_file, const char* rec_name,
    const char* label)
{
...
void
eh_close_data_file(Eh_data_file* data_file)
{
    if (data_file) {
        gssize i;
        eh_free(data_file->filename);
...
int
eh_get_data_file_size(Eh_data_file* data_file)
{
    gssize len = 0;
...
#ifdef OLD
Eh_data_record*
eh_get_data_from_file(Eh_data_file* data_file, int i)
{
