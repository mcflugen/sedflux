...
void
run_exponential_flow(Sed_column c, double time_now_in_years);
void
run_terzaghi_flow(Sed_column c, double time_now_in_years);
void
run_darcy_flow(Sed_column c, double dt_in_years);
...
init_flow(Sed_process p, Eh_symbol_table tab, GError** error)
{
    ...
    if (g_ascii_strcasecmp(key, "EXPONENTIAL") == 0) {
        data->method = FLOW_ALGORITHM_EXPONENTIAL;
    } else if (g_ascii_strcasecmp(key, "DARCY") == 0) {
        data->method = FLOW_ALGORITHM_DARCY;
    } else if (g_ascii_strcasecmp(key, "TERZAGHI") == 0) {
        data->method = FLOW_ALGORITHM_TERZAGHI;
    } else
        g_set_error(&tmp_err,
            SEDFLUX_ERROR,
...
gboolean
dump_flow_data(gpointer ptr, FILE* fp)
{
...
gboolean
load_flow_data(gpointer ptr, FILE* fp)
{
    Flow_t* data = (Flow_t*)ptr;
...
void
run_darcy_flow(Sed_column col, double dt_in_years)
{
    int j, n;
    double* load, *u, *k, *c;
