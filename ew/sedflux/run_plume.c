...
gboolean
plume3d(Plume_inputs* plume_const, Plume_river river,
    int n_grains, Plume_sediment* sedload,
    Eh_dbl_grid* deposit, Plume_data* data);
gboolean
compare_river(Plume_river* r1, Plume_river* r2, int n_grains);
int
copy_river(Plume_river* r1, Plume_river* r2, int n_grains);
    ...
    { /* Run the plume for all rivers */
        ...
        if (all_leaves) {
            for (r = all_leaves ; *r ; r++) {
                ...
                if (sed_river_is_hyperpycnal(*r)) {
                    eh_debug("Plume is hyperpycnal");
                } else {
                    eh_debug("Plume is hypopycnal");
                }
        ...
        */
        ...
        eh_debug("Calculate the input and output sediment");
        {
            ...
            */
            ...
            for (i = 0 ; i < n_susp_grains ; i++) {
                eh_message("river conc %d (kg/m^3): %f", i, sed_hydro_nth_concentration(hydro_data,
                        i));
            }
        }
...
init_plume_hypo(Sed_process p, Eh_symbol_table tab, GError** error)
{
    ...
    if (sed_mode_is_3d()) {
        eh_symbol_table_require_labels(tab, hypo_3d_req_labels, &tmp_err);
    } else {
        eh_symbol_table_require_labels(tab, hypo_2d_req_labels, &tmp_err);
    }

    if (!tmp_err) {
        if (sed_mode_is_3d()) {
            data->current_velocity = eh_symbol_table_input_value(tab, HYPO_KEY_CURRENT_VEL,
                    &tmp_err);
        } else {
...
init_plume_hypo_data(Sed_process proc, Sed_cube prof, GError** error)
{
    ...
    if (data) {
        data->last_deposit_grid = sed_cell_grid_new(2 * sed_cube_n_x(prof),
                2 * sed_cube_n_y(prof));
        data->deposit_grid      = sed_cell_grid_new(2 * sed_cube_n_x(prof),
                2 * sed_cube_n_y(prof));
...
gboolean
dump_plume_data(gpointer ptr, FILE* fp)
{
