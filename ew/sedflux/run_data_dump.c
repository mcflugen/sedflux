...
init_data_dump(Sed_process p, Eh_symbol_table tab, GError** error)
{
    ...
    if (eh_symbol_table_require_labels(tab, data_dump_req_labels, &tmp_err)) {
        ...
        if (strcasecmp(eh_symbol_table_lookup(tab, DATA_DUMP_KEY_Y_LIM), "tight") == 0) {
            ...
            gchar** y_lim_str = g_strsplit(eh_symbol_table_lookup(tab, DATA_DUMP_KEY_Y_LIM), ",",
                    2);
        ...
        if (strcasecmp(eh_symbol_table_lookup(tab, DATA_DUMP_KEY_X_LIM), "tight") == 0) {
            ...
            gchar** x_lim_str = g_strsplit(eh_symbol_table_lookup(tab, DATA_DUMP_KEY_X_LIM), ",",
                    2);
        ...
        {
            gint         i;
            gchar**      property = g_strsplit(eh_symbol_table_lookup(tab, DATA_DUMP_KEY_PROPERTY),
                    ",", -1);
        ...
        if (!tmp_err) {
            eh_open_dir(data->output_dir, &tmp_err);
        }
    }
...
gboolean
dump_data_dump_data(gpointer ptr, FILE* fp)
{
    guint len;
...
gboolean
load_data_dump_data(gpointer ptr, FILE* fp)
{
    Data_dump_t* data = (Data_dump_t*)ptr;
    Sed_property* property;
    guint len;
