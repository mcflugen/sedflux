...
init_inflow(Sed_process p, Eh_symbol_table tab, GError** error)
{
    ...
    if (!tmp_err) {
        ...
        */
        ...
        eh_check_to_s(data->sub >= 0, "Bottom sediment shear strength gradient positive",
            &err_s);
        ...
        eh_check_to_s(data->channel_width >= 0,
            "Channel width positive", &err_s);
        eh_check_to_s(data->channel_length >= 0,
            "Channel length positive", &err_s);
...
gboolean
dump_turbidity_current_data(gpointer ptr, FILE* fp)
{
...
gboolean
load_turbidity_current_data(gpointer ptr, FILE* fp)
{
    Inflow_t* data = (Inflow_t*)ptr;
...
double
getPhe(double* phe, double pos, double depth)
{
    Sed_cell avg;
    double volume;
    int i, n, n_grains;
