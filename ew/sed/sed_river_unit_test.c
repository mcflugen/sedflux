...
{
    {
        ...
        sed_river_split(r);
        fail_unless(sed_river_longest_branch(r) == sed_river_left(r),
            "Left branch in the case of a tie");

        sed_river_split(sed_river_left(r));
        sed_river_split(sed_river_right(sed_river_left(r)));
        fail_unless(sed_river_longest_branch(r) == sed_river_right(r),
            "Error finding longest branch");
    }
}
END_TEST
...
START_TEST(test_sed_river_set_angle)
{
    ...
    fail_unless(eh_compare_dbl(sed_river_angle(r), .25 * G_PI, 1e-12),
        "Angle incorrectly set");
