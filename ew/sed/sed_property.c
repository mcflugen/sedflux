...
typedef struct {
    ...
    f;      ///< The function used to get the property from a Sed_cell
...
Sed_property
sed_property_new_full(char* name, char* ext, Sed_cell_property_func f, gssize n_args)
{
    Sed_property p;
...
Sed_property
sed_property_copy(Sed_property dest, Sed_property src)
{
    eh_require(src);
...
Sed_property
sed_property_dup(Sed_property src)
{
    return sed_property_copy(NULL, src);
}

gboolean
sed_property_is_named(Sed_property p, const char* name)
{
    return g_ascii_strcasecmp(p->name, name) == 0;
}

char*
sed_property_extension(Sed_property p)
{
    char* ext = NULL;
...
char*
sed_property_name(Sed_property p)
{
    char* name = NULL;
...
gssize
sed_property_n_args(Sed_property p)
{
...
double
sed_property_measure(Sed_property p, Sed_cell c, ...)
{
    double val = 0.;
...
Sed_property
sed_property_destroy(Sed_property p)
{
    if (p) {
        eh_free(p->name);
        eh_free(p->ext);
