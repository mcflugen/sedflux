...
static Sed_hydrotrend_header*
_hydro_read_hydrotrend_header(Sed_hydro_file fp);
static Sed_hydro
_hydro_read_hydrotrend_record(Sed_hydro_file fp);
static Sed_hydro
_hydro_read_hydrotrend_record_buffer(Sed_hydro_file fp);
...
sed_hydro_new_from_table(Eh_symbol_table t, GError** error)
{
    ...
    if (t) {
        ...
        if (eh_symbol_table_require_labels(t, required_labels, &tmp_err)) {
            gint    n_susp_grains;
            double* c = eh_symbol_table_dbl_array_value(t, SED_HYDRO_LABEL_SUSPENDED_CONC,
                    &n_susp_grains, NULL);
...
sed_hydro_type_to_s(Sed_hydro_file_type t)
{
    ...
    switch (t) {
        case SED_HYDRO_INLINE        :
            s = __hydro_inline_type_s ;
            break;

        case SED_HYDRO_HYDROTREND    :
            s = __hydro_hydrotrend_type_s ;
            break;

        case SED_HYDRO_HYDROTREND_BE :
            s = __hydro_hydrotrend_be_type_s ;
            break;

        case SED_HYDRO_HYDROTREND_LE :
            s = __hydro_hydrotrend_le_type_s ;
            break;

        default                      :
            s = __hydro_unknown_type_s;
    }
...
Sed_hydro
sed_hydro_copy(Sed_hydro dest, Sed_hydro src)
{
...
sed_hydro_array_dup(Sed_hydro* src)
{
    ...
    if (src) {
        gint       i;
        gint       len = g_strv_length((gchar**)src);
        Sed_hydro* p   = src;

        dest = eh_new(Sed_hydro, len + 1);

        for (i = 0 ; *p ; p++, i++) {
            dest[i] = sed_hydro_dup(*p);
        }
...
Sed_hydro
sed_hydro_resize(Sed_hydro a, gssize n)
{
    if (!a) {
        a = sed_hydro_new(n);
    } else {
...
Sed_hydro
sed_hydro_destroy(Sed_hydro rec)
{
    if (rec) {
        eh_free(rec->conc);
        eh_free(rec);
...
gint
sed_hydro_fprint_rec(FILE* fp, Sed_hydro rec, const gchar* label)
{
    gssize n = 0;
    ...
    if (rec) {
        gssize i;
        const gchar* l = label ? label : "Hydro Record";

        n += fprintf(fp, "[ %s ]\n", l);
        n += fprintf(fp, "Duration                              : %fd\n", rec->duration);
        n += fprintf(fp, "Bedload (kg/s)                        : %f\n", rec->bedload);
        n += fprintf(fp, "Suspended load concentration (kg/m^3) : ");
...
*/
double*
sed_hydro_copy_concentration(double* dest, Sed_hydro a)
{
    if (!dest) {
...
double
sed_hydro_nth_concentration(Sed_hydro a, gssize n)
{
...
sed_hydro_is_hyperpycnal(Sed_hydro a)
{
    ...
    if (sed_hydro_flow_density(a, sed_rho_fresh_water()) > 1.01 * sed_rho_sea_water()) {
        is_hyper = TRUE;
    } else {
        is_hyper = FALSE;
    }
...
double
sed_hydro_suspended_concentration(Sed_hydro a)
{
    double total_conc = 0;
...
\returns The mass flux in units of kg/s
*/
double
sed_hydro_suspended_flux(Sed_hydro a)
{
...
double
sed_hydro_suspended_volume_flux(Sed_hydro a)
{
    double total = 0;
...
double
sed_hydro_water_flux(Sed_hydro a)
{
    eh_return_val_if_fail(a, 0);
...
double
sed_hydro_suspended_load(Sed_hydro a)
{
    eh_return_val_if_fail(a, 0);
...
Sed_hydro
sed_hydro_set_nth_concentration(Sed_hydro a, gssize n, double val)
{
    eh_return_val_if_fail(a, NULL);
...
Sed_hydro
sed_hydro_set_velocity(Sed_hydro a, double val)
{
    a->velocity = val;
    return a;
}
...
Sed_hydro
sed_hydro_set_width(Sed_hydro a, double val)
{
    a->width = val;
    return a;
}
...
Sed_hydro
sed_hydro_set_depth(Sed_hydro a, double val)
{
    a->depth = val;
    return a;
}
...
Sed_hydro
sed_hydro_set_bedload(Sed_hydro a, double val)
{
    a->bedload = val;
    return a;
}
...
double
sed_hydro_velocity(Sed_hydro a)
{
    return a->velocity;
}

double
sed_hydro_width(Sed_hydro a)
{
    return a->width;
}

double
sed_hydro_depth(Sed_hydro a)
{
    return a->depth;
}

double
sed_hydro_bedload(Sed_hydro a)
{
    return a->bedload;
}

double
sed_hydro_duration(Sed_hydro a)
{
    return a->duration;
}

double
sed_hydro_duration_in_seconds(Sed_hydro a)
{
...
Sed_hydro
sed_hydro_subtract_cell(Sed_hydro a,
    const Sed_cell s)
{
...
Sed_hydro
sed_hydro_average_records(Sed_hydro* rec, gssize n_recs)
{
    Sed_hydro mean_rec;
...

    {
        ...
        mean_rec->width  = total_q / mean_rec->duration / (mean_rec->velocity *
                mean_rec->depth);
...
double
sed_hydro_sum_durations(Sed_hydro* rec, gssize n_recs)
{
    double duration = 0;
...
gint
cmp_hydro_sort_vals(Hydro_sort_st* a, Hydro_sort_st* b) G_GNUC_INTERNAL;
gint
cmp_hydro_sort_inds(Hydro_sort_st* a, Hydro_sort_st* b) G_GNUC_INTERNAL;
...
sed_hydro_array_eventize_number(Sed_hydro* rec_a, gssize n_events)
{
    ...
    if (rec_a && n_events >= 0) {
        event_a = sed_hydro_process_records(rec_a, g_strv_length((gchar**)rec_a), n_events,
                TRUE);
    }
...
Sed_hydro*
sed_hydro_array_eventize_conc(Sed_hydro* rec_a, double c)
{
    Sed_hydro* event_a = NULL;

    if (rec_a && rec_a[0] && c >= 0) {
        {
            gssize     n_events  = 0;
            ...
            for (r = rec_a ; *r ; r++) {
                if (sed_hydro_flow_density(*r, sed_rho_fresh_water()) > c) {
                    n_events++;
                }
            }
...
{
    Sed_hydro* return_ptr = NULL;

    eh_return_val_if_fail(rec, NULL);
    eh_return_val_if_fail(rec[0], NULL);

    if (n_sig_values == 0 && insert_mean_values) {
        Sed_hydro mean_rec = sed_hydro_average_records(rec, n_recs);
        eh_strv_append((gchar***)&return_ptr, (gchar*)mean_rec);
    } else {
        GSList*    top_n   = NULL;
...
gint
cmp_hydro_sort_vals(Hydro_sort_st* a, Hydro_sort_st* b)
{
    if (a->val < b->val) {
        return -1;
    } else if (a->val > b->val) {
...
gint
cmp_hydro_sort_inds(Hydro_sort_st* a, Hydro_sort_st* b)
{
    if (a->ind < b->ind) {
        return -1;
    } else if (a->ind > b->ind) {
...
Sed_hydro_file
sed_hydro_file_set_wrap(Sed_hydro_file fp, gboolean wrap_is_on)
{
...
Sed_hydro_file
sed_hydro_file_set_buffer_length(Sed_hydro_file fp, gssize len)
{
...
Sed_hydro_file
sed_hydro_file_set_sig_values(Sed_hydro_file fp, int n_sig_values)
{
...
sed_hydro_file_header(Sed_hydro_file fp)
{
    return fp->hdr;
}
    ...
    if (filename) {
        ...
        if (strcmp(filename, "-") == 0) {
            ...
            if (type == SED_HYDRO_INLINE) {
                fp->fp = eh_fopen_error(filename, "r", &tmp_err);
            } else {
                fp->fp = eh_fopen_error(filename, "rb", &tmp_err);
            }
        }
...
*/
Sed_hydro_file
sed_hydro_file_destroy(Sed_hydro_file fp)
{
...
Sed_hydro*
sed_hydro_file_fill_buffer(Sed_hydro_file fp)
{
    int i;
