...
static gboolean __quit_signal =
    FALSE; //< signal to indicate that the user wishes to quit gracefully
static gboolean __exit_signal =
    FALSE; //< signal to indicate that the user wishes to quit ungracefully
static gboolean __dump_signal =
    FALSE; //< signal to indicate that the user wishes to dump output
static gboolean __user_signal =
    FALSE; //< signal to indicate that the user wishes to do something
static gboolean __next_signal =
    FALSE; //< signal to indicate that the user wishes to goto next epoch
...
sed_signal_is_pending(Sed_sig_num sig)
{
    ...
    if (sig == SED_SIG_QUIT && __quit_signal) {
        is_pending = TRUE;
    }

    if (sig == SED_SIG_EXIT && __exit_signal) {
        is_pending = TRUE;
    }

    if (sig == SED_SIG_DUMP && __dump_signal) {
        is_pending = TRUE;
    }

    if (sig == SED_SIG_CPR  && __cpr_signal) {
        is_pending = TRUE;
    }

    if (sig == SED_SIG_NEXT && __next_signal) {
        is_pending = TRUE;
    }

    if (sig == SED_SIG_USER && __user_signal) {
        is_pending = TRUE;
    }
...
_print_choices(int sig_num)
{
    ...
    for (msg = head_msg ; *msg ; msg++) {
        fprintf(stdout, "%s\n", *msg);
    }

    for (msg = body_msg ; *msg ; msg++) {
        fprintf(stdout, "%s\n", *msg);
    }

    for (msg = tail_msg ; *msg ; msg++) {
        fprintf(stdout, "%s\n", *msg);
    }
    ...
    if (sed_signal_is_pending(SED_SIG_EXIT)) {
