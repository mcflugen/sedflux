    ...
    GError** error)
{
...

    {
        ...
        if (!tmp_err) {
            sed_epoch_sscan_number(e, number_s, &tmp_err);
        }

        if (!tmp_err) {
            sed_epoch_sscan_duration(e, duration_s, &tmp_err);
        }

        if (!tmp_err) {
            sed_epoch_sscan_time_step(e, time_step_s, &tmp_err);
        }

        if (!tmp_err) {
            sed_epoch_sscan_filename(e, file_s, &tmp_err);
        }

        if (!tmp_err) {
            sed_epoch_sscan_prefix(e, prefix, &tmp_err);
        }
    ...
    if (time_s && time_step_s && file_s) {
        ...
        if (len_time == len_dt) {
            for (i = 0 ; !tmp_err && i < len_time ; i++) {
                ...
                if (!tmp_err) {
                    sed_epoch_sscan_time(e, time_str[i], &tmp_err);
                }

                if (!tmp_err) {
                    sed_epoch_sscan_time_step(e, dt_str[i], &tmp_err);
                }
            ...
            for (i = 0 ; !tmp_err && i < len_time ; i++) {
                ...
                if (!tmp_err) {
                    sed_epoch_sscan_time_step(e, dt_str[0], &tmp_err);
                }
    ...
    GError** error)
{
    ...
    if (file) {
        ...
        if (!tmp_err)
            sed_epoch_queue_set_processes(q,
...
sed_epoch_sscan_time(Sed_epoch e, const gchar* time_s, GError** error)
{
    ...
    if (e) {
        ...
        if (!tmp_err) {
            sed_epoch_set_active_time(e, time);
        }
...
sed_epoch_sscan_duration(Sed_epoch e, const gchar* duration_s, GError** error)
{
    ...
    if (e) {
        ...
        if (!tmp_err) {
            sed_epoch_set_duration(e, duration);
        }
