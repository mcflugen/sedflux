            ...
            if (h) {
                if (h->n_grains != hdr->n_grains && !tmp_err)
                    g_set_error(&tmp_err, SED_HYDROTREND_ERROR, SED_HYDROTREND_ERROR_BAD_HEADER,
                        "Number of grain sizes in Hydrotrend files do not match (%d!=%d)", hdr->n_grains,
                        h->n_grains);

                if (h->n_seasons != hdr->n_seasons && !tmp_err)
                    g_set_error(&tmp_err, SED_HYDROTREND_ERROR, SED_HYDROTREND_ERROR_BAD_HEADER,
                        "Number of seasons in Hydrotrend files do not match (%d!=%d)", hdr->n_seasons,
                        h->n_seasons);
    ...
    gint   order)
{
    ...
    if (fp) {
        ...
        if (order == G_BYTE_ORDER) {
            ...
            fwrite_int = eh_fwrite_int32_swap;
        }
...
Sed_hydro*
sed_hydrotrend_read_n_recs(const gchar* file, gint n_recs, gint byte_order,
    int* n_seasons, GError** error)
{
    Sed_hydro* arr = NULL;
